...
int main()
{
	int n;
	WSADATA wd;
	n = WSAStartup(MAKEWORD(2, 2), &wd);//初始化WS2_32.dll
	if (n)
	{
		cout << "WSAStartup函数错误！" << endl;
		return -1;
	}
	atexit(AutoWSACleanup);//自动调用WSACleanup
	//创建SOCKET
	SOCKET sock = socket(AF_INET, SOCK_RAW, IPPROTO_IP);//IPPROTO_IP表示接收所有IP包
	...
	//获取本机地址
	char  name[128];
	if (-1 == gethostname(name, sizeof(name)))//获取主机名
	{
		closesocket(sock);
		cout << WSAGetLastError();
		return 0;
	...
	pHostent = gethostbyname(name);//获取主机信息
	//绑定本地地址到SOCKET句柄
	sockaddr_in addr;//IP地址结构
	addr.sin_family = AF_INET;
	addr.sin_addr = *(in_addr*)pHostent->h_addr_list[0]; //IP
	addr.sin_port = 8888; //端口，IP层端口
	...
	do
	{
		len = recv(sock, buf, sizeof(buf), 0);//接收数据
		if (len > 0)
		{
			DecodeIP(buf, len);//解析IP包
		}
	} while (len > 0);
